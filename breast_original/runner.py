...
if scvi_is_trained is False:
    if setup.EXPRESSION_MODEL == "scanvi":
        scvi.model.SCANVI.setup_anndata(
            ...
            labels_key=f"{setup.CELL_TYPE}_coarse",
        )
...
if compute_mde:
    ...
    sc.pl.embedding(
        adata,
        basis=SCVI_MDE_KEY,
        color=[f"{setup.CELL_TYPE}_coarse", setup.BATCH],
        frameon=False,
        ncols=1,
        palette=PALETTE,
        show=False,
...
if compute_scib:
    ...
    bm = Benchmarker(
        adata,
        batch_key=setup.BATCH,
        label_key=f"{setup.CELL_TYPE}_coarse",
        embedding_obsm_keys=embedding_obsm_keys,
        bio_conservation_metrics=biocons,
        batch_correction_metrics=batchcorr,
        n_jobs=-1,
