...
from banksy.main import (
    generate_spatial_weights_fixed_nbrs,
    median_dist_to_nearest_neighbour,
)


def initialize_banksy(
    ...
    banksy_dict = (
        {}
    )  # Dictionary containing all the information across different parameter settings during runtime

    weights_graphs = {}  # Sub-dictionary containing weighted graph

    # Find median distance to closest neighbours
    nbrs = median_dist_to_nearest_neighbour(adata, key=coord_keys)

    for m in range(max_m + 1):

        weights_graph, distance_graph, theta_graph = (
            generate_spatial_weights_fixed_nbrs(
                adata.obsm[coord_keys],
                m=m,
