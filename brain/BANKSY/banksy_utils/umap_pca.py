    ...
    for nbr_weight_decay in banksy_dict:
        for lambda_param in banksy_dict[nbr_weight_decay]:
            ...
            for pca_dim in pca_dims:
                ...
                # save in AnnData object[obsm] attribute mentionning pca_dim and lambda_param:
                # ------------------

                if type_to_shuffle is not None:
                    adata.obsm[
                        f"banksy_pc_{pca_dim}_lambda_{lambda_param}_{type_to_shuffle.split()[0]}"
                    ] = reduced

                else:
                    adata.obsm[f"banksy_pc_{pca_dim}_lambda_{lambda_param}"] = reduced
                ...
                if add_umap:
                    print(
                        f'Conducting UMAP and adding embeddings to adata.obsm["reduced_pc_{pca_dim}_umap"]'
                    )
