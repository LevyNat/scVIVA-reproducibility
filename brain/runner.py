...
for setting in niche_setup.keys():
    ...
    if compute_umap:
        ...
        plt.savefig(
            f"{setup.FIGURES_FOLDER}{setting}_umap.png", bbox_inches="tight", dpi=1000
        )
        plt.savefig(
            f"{setup.FIGURES_FOLDER}{setting}_umap.svg", bbox_inches="tight", dpi=1000
        )
...
if compute_scib:
    ...
    plt.savefig(
        f"{setup.FIGURES_FOLDER}scib_cell_type.png", bbox_inches="tight", dpi=800
    )
    plt.savefig(
        f"{setup.FIGURES_FOLDER}scib_cell_type.svg", bbox_inches="tight", dpi=800
    )
    ...
    #         np.save(
    #             f"{setup.FIGURES_FOLDER_SHUFFLE}cLISI_{type.split()[0]}_{key}.npy",
    #             clisi_key,
    #         )
