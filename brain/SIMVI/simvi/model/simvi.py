...
class SimVIModel(BaseModelClass):
    ...
    def extract_edge_index(
        ...
        if batch_key is not None:
            ...
            for i in adata.obs[batch_key].unique():
                ...
                if method == "knn":
                    A = kneighbors_graph(
                        adata_tmp.obsm[spatial_key], n_neighbors=n_neighbors
                    )
                    edge_index_tmp, edge_weight = from_scipy_sparse_matrix(A)
                    label = torch.arange(adata.shape[0])[
                        adata.obs_names.isin(adata_tmp.obs_names)
                    ]
    ...
    def get_latent_representation(
        ...
        outputs = self.module.inference(
            data[np.arange(data.get_data("X").shape[0])], edge_index
        )
        latent = []
    ...
    def get_decoded_expression(
        ...
        decoded = []
        if representation_kind == "intrinsic":
            decoded_m = self.module._generic_generative(
                outputs["q_m"], outputs["library"], outputs["batch_index"]
            )["px_scale"]
            decoded_sample = self.module._generic_generative(
                outputs["z"], outputs["library"], outputs["batch_index"]
            )["px_scale"]

        elif representation_kind == "interaction":
            decoded_m = self.module._generic_generative(
                outputs["qgat_m"], outputs["library"], outputs["batch_index"]
            )["px_scale"]
            decoded_sample = self.module._generic_generative(
                outputs["z_gat"], outputs["library"], outputs["batch_index"]
            )["px_scale"]

        elif representation_kind == "all":
            decoded_m = self.module._generic_generative(
                outputs["qall_m"], outputs["library"], outputs["batch_index"]
            )["px_scale"]
            decoded_sample = self.module._generic_generative(
                outputs["z_all"], outputs["library"], outputs["batch_index"]
            )["px_scale"]
    ...
    def get_spatial_effect(
        ...
        assert mode in available_modes, (
            f"mode = {mode} is not one of" f" {available_modes}"
        )
        ...
        if mode == "original":
            ...
            if give_mean:
                ...
                decoded_null = self.module._generic_generative(
                    torch.cat(
                        (
                            outputs["q_m"],
                            outputs["qgat_m"] * 0
                            + outputs["qgat_m"].mean(axis=0)[None, :],
                        ),
                        1,
                    ),
                    outputs["library"],
                ...
                decoded_null = self.module._generic_generative(
                    torch.cat(
                        (
                            outputs["z"],
                            outputs["z_gat"] * 0
                            + outputs["z_gat"].mean(axis=0)[None, :],
                        ),
                        1,
                    ),
                    outputs["library"],
    ...
    @torch.no_grad()
    def get_attention(self, edge_index) -> np.ndarray:
        data = AnnTorchDataset(self.adata_manager)
        inference_input = self.module._get_inference_input_from_concat_tensors(
            data[np.arange(data.get_data("X").shape[0])]
        )
        q = self.module.base_encoder.encoder(
            inference_input["x"], inference_input["batch_index"]
        )
        q_l = self.module.gat_mean.lin_l(q)
        q_r = self.module.gat_mean.lin_r(q)
        x = q_l.view((q_l.shape[0], 1, q_l.shape[1])) + q_r.view(
            (1, q_r.shape[0], q_r.shape[1])
        )
    ...
    def train(
        ...
        for epoch in pbar:
            train_loss.append(
                _train(
                    self.module, data, edge_index, train_mask, optimizer, batch_size
                ).detach()
            )
            val_loss.append(_eval(self.module, data, edge_index, val_mask).detach())
            pbar.set_description("Epoch " + str(epoch) + "/" + str(max_epochs))
            pbar.set_postfix(
                train_loss=train_loss[epoch - 1].numpy(),
                val_loss=val_loss[epoch - 1].numpy(),
            )
            # print('Epoch ',epoch)
        return train_loss, val_loss, train_mask, val_mask
...
def _train(model, data, edge_index, mask, optimizer, batch_size):
    ...
    if batch_size is None:
        ...
        batch_indices = [
            mask[i : i + batch_size] for i in range(0, mask.shape[0], batch_size)
        ]
        ...
        for batch_index in batch_indices:
            ...
            decoder_dict = model.generative(latent_dict_masked)
            lossrecorder = model.loss(
                data[batch_index], latent_dict_masked, decoder_dict
            )
            loss = lossrecorder.loss
            loss.backward()
            optimizer.step()
...
def _prob(loc, scale, value):
    ...
    log_prob = (
        -((value[None, :] - loc) * (value[None, :] - loc)) / (2 * var)
        - log_scale
        - torch.log(torch.tensor(math.sqrt(2 * math.pi)))
    )
